...
The following activity diagram summarizes what happens when a user executes an undoable command:
...
* **Alternative 1: (current choice)** Implements undo and redo of each Command in a separate Event object.
** Pros:
...
* **Alternative 3 (current choice):** Saves the entire address book.
** Pros:
*** Easy to implement.
** Cons:
*** May have performance issues in terms of memory usage.
*** Very unnecessary storage of irrelevant details such as schedule list.


===== Aspect: Data structure to support the undo/redo commands

* **Alternative 1:** Use `EventHistory` to store undo and redo stacks
** Pros:
*** We do not need to maintain a separate list, and just reuse what is already in the codebase.

* **Alternative 2:** Use a list to store the history of address book states.
