...
After. Exercise now appears at the bottom as the list is sorted by date.
...
**Step 2**: `undo` +
The exercise `Hiking` is added back to the regime `Level 3` +
...
Redo the previous command that was undone by user. Since a redo command can only be executed after
successful executions of the undo command, it makes sense that only Undoable commands can be redone.
For a list of Undoable commands, refer to the <<undocommand, Undo Command>>.
